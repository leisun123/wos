...
## 九、第五轮调试记录（2026-08-25 深夜）：搜索条闭环攻坚

### 关键认知（本轮最大收获）

1. **搜索条是矿详情弹窗的一部分**：弹窗打开时底部行为「资源图标(58,1960) | 等级下拉(175,1960) | 勾选框(270,1960) | 搜索按钮(650,1960)」；**弹窗关闭后同一位置是聊天按钮/输入框**——此前反复点搜索开出聊天的根本原因。
2. **世界地图面板层叠严重**：联盟聊天（新消息自动弹横幅）+ 置顶公告（点击左下区域触发）+ 促销弹窗此起彼伏。已知关闭方式：聊天=右上X(1010,230)；公告=底部「世界地图」标签(250,2320)；退出确认=取消。
3. 「查看」跳转按钮 (237,288) 仅在**队列面板展开**时有效；面板收起时该位置是地图。
4. 队列面板展开/收起状态不定——入口点击前必须先确认状态。

### 待实现的完整闭环（设计定稿）

```
状态机循环（每步截图验证，任何面板先清理）:
  world: 队列展开? -> 点查看(237,288) 跳到采集矿
         队列收起? -> 点队列区展开 -> 重试
  mine_detail(储量+采集):
         勾选满资源(270,1960) -> 搜索(650,1960) -> 新满矿详情
         或直接 采集(OCR定位) -> 出征面板 -> 平均配置 -> 出征
  deploy_panel: 平均配置 -> 出征 -> 验证行军
  聊天/公告/促销/退出框: 各自关闭 -> 回 world
循环直到 行军x/y 满 -> 等最早倒计时归队 -> 继续
```

### 下轮任务

1. 把上述状态机补进 tools/gather_cn.py（加聊天/公告清理分支）跑通多矿循环
2. 搜索前先勾选、验证搜索跳转到满矿详情（等级/储量 OCR 校验）
3. 资源配比选择（搜索条资源图标下拉菜单未验证）
4. 归队重派：顶部倒计时监控 + 归队后重新派遣
